...
std::unique_ptr<Cipher> create_cipher(CipherType type) {
    ...
    case CipherType::BLOWFISH:
        return std::make_unique<BlowfishSimple>();
...
std::string cipher_type_to_string(CipherType type) {
    ...
    case CipherType::BLOWFISH: return "Blowfish"; 
...
CipherType string_to_cipher_type(const std::string& str) {
    ...
    if (str == "Blowfish" || str == "blowfish") return CipherType::BLOWFISH;
